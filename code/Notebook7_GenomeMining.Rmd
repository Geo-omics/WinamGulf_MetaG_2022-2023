...
#Results found here: /geomicro/data21/lnhart/Projects/Kenya_2023/flagship_manuscript/FINAL2/results/antismash_drep_BGCs/bigscape2
...
#/geomicro/data21/lnhart/Projects/Kenya_2023/flagship_manuscript/FINAL2/results/antismash_drep_BGCs/extractedbgcs/extractedbgcs_95_90_rep_seq/extractedbgcs_95_90_rep_seq.gbk 
...
sample_table = read.csv("/geomicro/data21/lnhart/Projects/Kenya_2023/flagship_manuscript/FINAL2/tables/GLOD_LH_Mar52024.csv") 
...
##Fix RPKM value to be FPKM based on https://www.biostars.org/p/273537/
#Difference between our value used and the correct FPKM value is actually just a scale of 2000
bgc70_meta = bgc70_meta %>% mutate(FPKM = rpkm*2000)

##############3. Normalize for duplicates in 2022 samples

bgc70_meta22 = bgc70_meta %>% filter(year == 2022)  #1778
bgc70_meta22 = bgc70_meta22 %>% group_by(site_name_sitenumber, seqnames) %>% mutate(summed_fpkm = mean(FPKM), total_bgc_covered = mean(percent_cover))
dim(bgc70_meta22)
#[1] 1778   64

bgc70_meta22 = unique(bgc70_meta22[c('site_name_sitenumber', 'seqnames', 'summed_fpkm', 'total_bgc_covered')])
dim(bgc70_meta22)
#[1] 938   4
...
#get 2023 data 
bgc70_meta23 = bgc70_meta %>% ungroup() %>% filter(year == 2023) %>% select(site_name_sitenumber, seqnames, FPKM, percent_cover, year)
bgc70_meta23 = bgc70_meta23 %>% mutate(summed_fpkm = FPKM) %>% select(!FPKM) %>% mutate(total_bgc_covered = percent_cover) %>% select(!percent_cover)
dim(bgc70_meta23)
#[1] 2132    5
...
bgc_data <- bgc70_metanorm %>% ungroup() %>% select(c(grouping, site_name_sitenumber, summed_fpkm))
bgc_data_summed <- bgc_data %>%
  group_by(site_name_sitenumber,grouping) %>%
  summarise(summed_fpkm = sum(summed_fpkm, na.rm = TRUE)) %>%
  pivot_wider(names_from = grouping, values_from = summed_fpkm)
...
bgc_data_sum = bgc_data %>% group_by(site_name_sitenumber,grouping) %>%
  summarise(summed_fpkm = sum(summed_fpkm))
...
bgc_data_sum %>% filter(!grouping == "#VALUE!") %>% 
  ggplot(aes(y = site_name_sitenumber, x = grouping, size = summed_fpkm, fill = grouping)) +
  geom_point(shape = 21) +
  scale_fill_manual(values = col_vector) +
  scale_size_continuous(range = c(1, 10)) +
  labs(y = "Sampling Site", x = "Biosynthetic Gene Cluster (BGC)", fill = "Genus of BGC", size = "Total FPKM") +
  theme_bw() +
...
bgc_data_sum_updated2 %>% filter(!grouping == "#VALUE!") %>% 
  ...
  geom_point(shape = 21) +
  scale_fill_manual(values = col_vector) +
  scale_size_continuous(range = c(2, 10), breaks = c(250, 500, 750, 1000)) +
  labs(y = "Sampling Site", x = "Biosynthetic Gene Cluster (BGC) Type", size = "Total FPKM") +
  theme_light() +
