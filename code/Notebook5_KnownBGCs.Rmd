...
```{r}
#Calculate FPKM value based on more correct RPKM value calculation:  https://www.biostars.org/p/273537/
#Difference between our value used and the correct FPKM value is actually just a scale of 2000
bam_stat_rpkm70_key = bam_stat_rpkm70_key %>% mutate(FPKM = rpkm*2000)
...
bam_stat_rpkm70_key_cov_whole = bam_stat_rpkm70_key_cov %>% group_by(sample_id, New_BGC_name, tot_gene_lengths, FPKM) %>% summarize(bases_covering_bgc = sum(bases_covered_gene), total_fpkm = sum(FPKM), percent_bgc_covered = (bases_covering_bgc/tot_gene_lengths)*100)

#Calculate % of bgc covered in each sample 
bam_stat_whole_70_all = bam_stat_rpkm70_key_cov_whole %>% group_by(sample_id, New_BGC_name) %>% mutate(total_bgc_covered = sum(percent_bgc_covered), summed_fpkm = sum(FPKM))

#Pull distinct values for sample id and new bgc name 
bam_whole_summary = unique(bam_stat_whole_70_all[c('sample_id', 'New_BGC_name', 'summed_fpkm', 'total_bgc_covered')])
dim(bam_whole_summary)
#[1] 590   4

#Calculating how much of BGC is there based on # of genes 70% covered in the read mapping

#bam_stat_rpkm70_key_cov_count = bam_stat_rpkm70_key_cov %>% group_by(sample_id, New_BGC_name, tot_genes, FPKM) %>% summarize(num_genes_present = sum(gene_present == TRUE), percent_genes_pres = (num_genes_present/tot_genes)*100, tot_fpkm = sum(FPKM))
...
bam_markerbgc_whole_50 %>%
  ggplot(aes(x = site_name_sitenumber, y = New_BGC_name, fill = New_BGC_name, size = summed_fpkm)) +
  geom_point(shape = 21) +
  scale_fill_manual(values = col_vector) +
  scale_size_continuous(range = c(2, 8)) +
  labs(x = "Site Name Sitenumber", y = "BGC", fill = "BGC", size = "Total Fragments Per Kilobase Per Million Mapped Reads (FPKM)") +
  theme_bw() +
...
meta_bam22 = meta_bam22 %>% group_by(site_name_sitenumber, New_BGC_name) %>% mutate(summed_fpkm = mean(summed_fpkm), total_bgc_covered = mean(total_bgc_covered))
#103, 47

meta_bam22 = unique(meta_bam22[c('site_name_sitenumber', 'New_BGC_name', 'summed_fpkm', 'total_bgc_covered')])
dim(meta_bam22)
#68,4
...
#get 2023 data 
meta_bam23 = meta_bam_summary_complete_all %>% ungroup() %>% filter(year == 2023) %>% select(site_name_sitenumber, New_BGC_name, summed_fpkm, total_bgc_covered, year)
...
meta_bam22_70 = meta_bam22_70 %>% group_by(site_name_sitenumber, New_BGC_name) %>% mutate(summed_fpkm = mean(summed_fpkm), total_bgc_covered = mean(total_bgc_covered))
dim(meta_bam22_70)
#80, 47

meta_bam22_70 = unique(meta_bam22_70[c('site_name_sitenumber', 'New_BGC_name', 'summed_fpkm', 'total_bgc_covered')])
dim(meta_bam22_70)
#55,4
...
#get 2023 data 
meta_bam23_70 = meta_bam_summary_complete_all_70 %>% ungroup() %>% filter(year == 2023) %>% select(site_name_sitenumber, New_BGC_name, summed_fpkm, total_bgc_covered, year)
...
meta_bam_70 %>%
  ggplot(aes(y = site_name_sitenumber, x = easy_bgc_name, fill = BGC_genus, size = summed_fpkm)) +
  geom_point(shape = 21) +
  scale_fill_manual(values = col_vector) +
  scale_size_continuous(range = c(2, 6)) +
  labs(y = "Sampling Site", x = "Biosynthetic Gene Cluster (BGC)", fill = "Genus of BGC", size = "FPKM") +
  theme_bw() +
...
ggsave(ggsave("/geomicro/data21/lnhart/Projects/Kenya_2023/flagship_manuscript/FINAL/figures/bgc_mining/allbgcs_fixedcov_70_reversed_facet.png",width = 12, height =10, dpi=300, scale =2, limitsize = FALSE))

ggsave(ggsave("/geomicro/data21/lnhart/Projects/Kenya_2023/flagship_manuscript/FINAL/figures/bgc_mining/allbgcs_fixedcov_70_reversed_facet.svg",width = 12, height =10, dpi=300, scale =2, limitsize = FALSE))

ggsave(ggsave("/geomicro/data21/lnhart/Projects/Kenya_2023/flagship_manuscript/FINAL/figures/bgc_mining/allbgcs_fixedcov_70_reversed_facet.tiff",width = 12, height =10, dpi=300, scale =2, limitsize = FALSE))
```
